...
const fmtHM = s => `${String((s / 3600 | 0)).padStart(2, "0")}:${String((s % 3600 / 60 | 0)).padStart(2, "0")}`;
...
async function loadFiles() {
  ...
  if (!d.files || d.files.length === 0) {
    tbody.innerHTML = '<tr><td colspan="3" style="text-align:center;color:#777;">Keine Dateien gefunden</td></tr>';
    return;
  }
...
async function loadFooter(d) {
  ...
  f.innerHTML = `${fmtTime(utc)} · ${ind} · ${freeKB}KB (${fmtHM(free / data.sizePoint)}) frei`;
...
async function loadStatus() {
  const fixstr = ["-", "GPS", "DGPS", "PPS", "RTK", "FloatRTK", "Estimated", "Manual", "Simulated"];
  const r = await fetch(url + '/info');
  ...
  if (typeof d.logMode !== 'undefined') {
    ...
    const i = document.querySelector(`#logModeForm input[name=logmode][value='${v}']`);
    if (i) i.checked = true;
  }
...
window.addEventListener('load', () => {
  ...
  if (document.querySelector('#fileTable')) {
    ...
    if (toggleBtn) { if (!toggleBtn.dataset.orig) toggleBtn.dataset.orig = toggleBtn.innerHTML; toggleBtn.addEventListener('click', async () => { const newMode = window.currentLoggingActive ? 1 : 2; setButtonLoading(toggleBtn, true, 'Bitte warten...'); try { const r = await fetch(url + `/setlogactive?logActive=${newMode}`); if (r.ok) { await loadFilesAndFooter(); showSnackbar('OK', 'success', 2000) } else showSnackbar('Fehler beim Umschalten', 'error') } catch (e) { showSnackbar('Netzwerkfehler', 'error') } setButtonLoading(toggleBtn, false) }) }
  }
  ...
  if (document.querySelector('#statusInfo')) {
    ...
    if (saveBtn) { if (!saveBtn.dataset.orig) saveBtn.dataset.orig = saveBtn.innerHTML; saveBtn.addEventListener('click', async () => { const sel = document.querySelector('#logModeForm input[name=logmode]:checked'); const mode = sel.value; setButtonLoading(saveBtn, true, 'Speichern...'); try { const r = await fetch(url + `/setlogmode?logMode=${mode}`); if (r.ok) { showSnackbar('Gespeichert', 'success'); await loadStatus(); await loadFooter() } else showSnackbar('Fehler beim Speichern', 'error') } catch (err) { showSnackbar('Netzwerkfehler', 'error') } setButtonLoading(saveBtn, false) }) }
  }
  ...
  if (!window._footerIntervalID && document.querySelector('#footer')) {
    window._footerIntervalID = setInterval(() => {
      try { loadFooter(); } catch (e) { }
    }, 3000);
  }
});

// clear interval on unload
window.addEventListener('beforeunload', () => { if (window._footerIntervalID) { clearInterval(window._footerIntervalID); window._footerIntervalID = 0; } });
