...
async function loadFiles() {
  ...
  d.files.forEach((file) => {
    ...
    if (file["active"]) {
      ...
      tr.innerHTML = `
      <td>⬇️ <a class="download" href="${url}/download?file=${path}&tz=${tz}">${dname}</a></td>
...
async function loadFooter(d) {
  ...
  f.innerHTML = `${fmtTime(utc)} · ${ind} · ${freeKB}kB frei`;
...
async function loadStatus() {
  ...
  add("RAM frei (min)", Math.round(d.RAMminFree / 1024) + " kB");
