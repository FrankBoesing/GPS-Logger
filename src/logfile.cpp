...
void cFileWrite::open(const time_t time)
{
    if (f)
        return;
    ...
    if (RESTART_AFTER_IDLE) // An letztes Logfile anfügen?
    {
        ...
        if (found && (time - lastWritten <= MAX_IDLE_SECONDS))
        {
            append = true;
            log_i("Füge an Logfile an. Alter: %llds", time - lastWritten);
        }
    }

    if (!append)
        snprintf(filename, sizeof(filename), FILE_PREFIX "%lld" FILE_SUFFIX, time);
